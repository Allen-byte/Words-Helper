...
class Helper:
    ...
    url = "https://www.iloveyou3000.cn/Words-Helper/version.html"      # 在线检查软件版本地址
    download_url = "https://www.iloveyou3000.cn/Words-Helper/Words-Helper.exe" # 软件下载地址
    header = {
        "User-Agent":
    ...
    def download(self, url, file_path):
        ...
        @file_path:文件保存路径
        """
        res = requests.get(url=self.download_url, stream=True, headers=self.header)
    ...
    def update(self):
        ...
        :return:
        """
        soft_data = requests.get(url=self.url, headers=self.header).text
        html = etree.HTML(soft_data)
        info = html.xpath('//p/text()')[0].replace('\r\n', '').strip()
        info = info.split('=')[1].strip()            # 检查在线版本号
        new_software_file = os.getcwd() + f'\\Words-Helper{info}.exe'  # 新版本的下载路径
        self.config.read('info.ini')
        edition = self.config.get('init', '软件版本')         # 查看本地版本号
        if edition != info:
            self.config.set('init', '软件版本', info)
            self.config.write(open(self.info_file, 'w'))
            print('检测到新版本，开始下载...')
            self.download(self.download_url, new_software_file)
            print('下载完成，请尽快安装新版本以获得最佳体验')
            time.sleep(3)
        else:
            print('--->当前已是最新版本')
            time.sleep(2)
